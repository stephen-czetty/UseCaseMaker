...
namespace UseCaseMakerLibrary.Tests.XmlSerializeTests
{
    ...
    public class When_deserializing_from_xml
    {
        ...
        private It Should_throw_exception_when_major_version_is_too_low =
            () => Catch.Exception(() => XmlSerializer.XmlDeserialize(_document, "mainNode", "", "0.0", _model)).ShouldBeOfType<XmlSerializerException>();

        private It Should_throw_exception_when_minor_version_is_too_low = () => Catch.Exception(() => XmlSerializer.XmlDeserialize(_document, "mainNode", "", "1.0", _model)).ShouldBeOfType<XmlSerializerException>();
